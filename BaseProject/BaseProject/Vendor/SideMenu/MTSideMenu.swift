...
extension UIViewController{
    var sideMenuViewController: MTSideMenu? {
        get {
            return getSideViewController(self)
        }
    }
    
    private func getSideViewController(viewController: UIViewController) -> MTSideMenu? {
        if let parent = viewController.parentViewController {
            if parent is MTSideMenu {
                return parent as? MTSideMenu
            }else {
...
@objc protocol MTASideMenuDelegate {
    
    optional func sideMenuDidRecognizePanGesture(sideMenu: MTSideMenu, recongnizer: UIPanGestureRecognizer)
    optional func sideMenuWillShowMenuViewController(sideMenu: MTSideMenu, menuViewController: UIViewController)
    optional func sideMenuDidShowMenuViewController(sideMenu: MTSideMenu, menuViewController: UIViewController)
    optional func sideMenuWillHideMenuViewController(sideMenu: MTSideMenu, menuViewController: UIViewController)
    optional func sideMenuDidHideMenuViewController(sideMenu: MTSideMenu, menuViewController: UIViewController)
    
}

class MTSideMenu: UIViewController, UIGestureRecognizerDelegate {
    
    enum MTASideMenuPanDirection: Int {
        case Edge = 0
        case EveryWhere = 1
    }
    
    enum MTASideMenuType: Int {
        case Scale = 0
        case Slip = 1
    }
    
    enum MTAStatusBarStyle: Int {
        case Hidden = 0
        case Black = 1
        case Light = 2
    }
    
    private enum MTASideMenuSide: Int {
        case Left = 0
        case Right = 1
    }
...
//    }
    ...
    struct SideMenuOptions {
        ...
        init(animationDuration: Float = 0.35, panGestureEnabled: Bool = true, panDirection: MTASideMenuPanDirection = .Edge, type: MTASideMenuType = .Scale, panMinimumOpenThreshold: UInt = 60, menuViewControllerTransformation: CGAffineTransform = CGAffineTransformMakeScale(1.5, 1.5), backgroundTransformation: CGAffineTransform = CGAffineTransformMakeScale(1.7, 1.7), endAllEditing: Bool = false) {
    ...
    @IBInspectable var statusBarStyle: MTAStatusBarStyle = .Black
    ...
    @IBInspectable var panDirection: MTASideMenuPanDirection = .Edge
    @IBInspectable var type: MTASideMenuType = .Scale
    ...
    weak var delegate: MTASideMenuDelegate?
    ...
    private func showMenuViewController(side: MTASideMenuSide, menuViewController: UIViewController) {
    ...
    private func animateMenuViewController(side: MTASideMenuSide) {
    ...
    private func animateMenuViewControllerCompletion(side: MTASideMenuSide, menuViewController: UIViewController) {
